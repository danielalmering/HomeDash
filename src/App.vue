...
import { getParameterByName, storageFactory } from "./util";
...
export default class Cookies extends Vue {
  ...
  localStore = storageFactory(window.localStorage);
  sessionStore = storageFactory(window.sessionStorage);
  ...
  mounted() {
    ...
    const gotsafe = safeMode
      ? this.$store.commit("activateSafeMode")
  ...
  async created() {
    ...
    // Geo Safe check
    const geoResult = await fetch(`${config.BaseUrl}/loc`, {
      credentials: "include"
    });
    ...
    /*try {
      ...
      const cookiesAccepted =
        window.localStorage.getItem(`${config.StorageKey}.cookiesAccepted`) !==
        null
          ? window.localStorage.getItem(`${config.StorageKey}.cookiesAccepted`)
          : false;
      ...
      // Agecheck check
      const AgeCheckAccepted =
        window.localStorage.getItem(`${config.StorageKey}.agecheck`) !== null
          ? window.localStorage.getItem(`${config.StorageKey}.agecheck`)
          : false;
      this.displayAgecheck = !config.locale.AgeCheck
        ? false
        : !(AgeCheckAccepted && AgeCheckAccepted === "true");
    } catch (error) {
      if (
    ...
    const cookiesAccepted =
        this.localStore.getItem(`${config.StorageKey}.cookiesAccepted`) !==
        null
          ? this.localStore.getItem(`${config.StorageKey}.cookiesAccepted`)
          : false;
      this.displayCookies = !(cookiesAccepted && cookiesAccepted === "true");
      ...
      // Agecheck check
      const AgeCheckAccepted =
        this.localStore.getItem(`${config.StorageKey}.agecheck`) !== null
          ? this.localStore.getItem(`${config.StorageKey}.agecheck`)
          : false;
      this.displayAgecheck = !config.locale.AgeCheck
        ? false
        : !(AgeCheckAccepted && AgeCheckAccepted === "true");
    ...
    function registerHotjarToSentry() {
      ...
      if (
        hj &&
        hj.pageVisit &&
        hj.pageVisit.property &&
        hj.pageVisit.property.key
      ) {
